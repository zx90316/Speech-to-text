...
- 郵箱驗證：`backend/email_service.py`
- 管理員驗證：`backend/api.py` → `verify_admin_token()`
...
- `backend/input_validator.py` - 所有驗證邏輯
...
- `backend/security_logger.py` - 日誌記錄系統
...
- `backend/rate_limiter.py` - 速率限制器
- `backend/api.py` - `@limiter.limit()` 裝飾器
...
- `backend/api.py` → `add_security_headers()` 中介軟體
...
- `backend/input_validator.py` → `validate_upload_file()`
- `backend/memory_storage.py` → 文件管理
...
- `backend/memory_storage.py` → 數據管理
- `backend/crypto_utils.py` → `mask_email()`
- `backend/security_logger.py` → `log_data_deletion()`
